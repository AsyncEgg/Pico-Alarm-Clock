...
def checktime():
    ...
    if len(str(minute)) == 1:
        if '0'+str(minute) == atime.split(':')[1] and str(hour) == atime.split(':')[0]:
            alarm()
    if len(str(minute)) == 2:
        if str(minute) == atime.split(':')[1] and str(hour) == atime.split(':')[0]:
            alarm()
    if str(minute) in ['0','10','20','30','40','50']:
        sleep(1)
        byteimg(TH2,False)
